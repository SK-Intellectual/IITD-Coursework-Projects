...
int main() {
     ...
     for(int i=0;i<6;i++){
        ...
        out<<pow(kd[i],2)<<" "<<(fp[i]+rel_correction_factor)*(pow((1-(2*kd[i])),4))<<endl;
        outa<<pow(kd[i],2)<<" "<<(fpa[i]+rel_correction_factor)*(pow((1-(2*kd[i])),4))<<endl;
        out1<<pow(kd[i],2)<<" "<<fp1[i]*(pow((1-(2*kd[i])),4))<<endl;
        out1a<<pow(kd[i],2)<<" "<<fp1a[i]*(pow((1-(2*kd[i])),4))<<endl;
     }
    ...
    " 'g12a.txt' with linespoints lc rgb 'green' title 'Re = 1800, lambda/w = 8',"
    " 'g12.txt' with linespoints lc rgb 'red' title 'Re = 1800, lambda/w = 4'\"";
