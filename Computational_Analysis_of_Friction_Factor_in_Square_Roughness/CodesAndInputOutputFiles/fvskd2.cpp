...
int main() {
     ...
     for(int i=0;i<6;i++){
        ...
        out<<pow(kd[i],2)<<" "<<(fp[i]+rel_correction_factor)<<endl;
        outa<<pow(kd[i],2)<<" "<<(fpa[i]+rel_correction_factor)<<endl;
        out1<<pow(kd[i],2)<<" "<<fp1[i]<<endl;
        out1a<<pow(kd[i],2)<<" "<<fp1a[i]<<endl;
     }
    string command1 = "gnuplot -persist -e \""
    "set xlabel '(k/D)^2';"
    "set ylabel 'f/(64/Re)';"
    "plot 'g11.txt' with linespoints lc rgb 'red' title 'Re = 2000, lambda/w = 2',"
    " 'g11a.txt' with linespoints lc rgb 'blue' title 'Re = 2000, lambda/w = 4',"
    " 'g12a.txt' with linespoints lc rgb 'blue' title 'Re = 1800, lambda/w = 4',"
    " 'g12.txt' with linespoints lc rgb 'red' title 'Re = 1800, lambda/w = 2'\"";
